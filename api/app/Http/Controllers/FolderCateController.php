...
folder_cate_icon,folder_cate_grant_privileges,folder_category.mission_type_id,mt.mission_type_name,map,mission_begin_date,mission_complete_date
	...
	{		
		$items = DB::select("SELECT id,folder_cate_name,folder_cate_detail,map,mission_begin_date,mission_complete_date,folder_cate_seq FROM folder_category where id=? and folder_cate_status!=0 ",
		array($id));
		return response()->json($items);
	}
