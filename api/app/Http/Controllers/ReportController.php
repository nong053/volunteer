...
class ReportController extends Controller
{
	...
	{
		$items = DB::select("
				SELECT usage_log.log_id, concat(title,' ',first_name,'',last_name) as full_name,
				usage_log.created_dttm FROM profile p inner join usage_log  
				on p.profile_id=usage_log.profile_id
				where title like ? or first_name like  ? or last_name like ? 
				order by usage_log.created_dttm desc
		",array('%'.$request->expressSearch.'%','%'.$request->expressSearch.'%','%'.$request->expressSearch.'%'));
